...
// DOM элементы (все, что есть в HTML)
...
// ==================== ДОБАВЛЕНИЕ ПОВТОРЕНИЯ (С ПУЛЬСАЦИЕЙ, ЗВУКОМ, ФОНОМ) ====================
function addRep() {
    const ex = exercises[currentExercise];
    if (!ex || ex.isPlank) return;
    repCount++;
...
function onResults(results) {
    ...
    if (finalAngle < downThr && !isDown) { isDown=true; if (stateEl) stateEl.textContent="НИЗ"; }
    else if (finalAngle > upThr && isDown) {
        addRep();
        isDown=false; if (stateEl) stateEl.textContent="ВЕРХ";
...
async function startCameraAnalysis() {
    ...
    try {
        ...
        isAnalyzing=true; isPaused=false; if (pauseBtn) pauseBtn.textContent='⏸️ Пауза';
...
function resetCounter() {
    repCount=0; if (counterEl) counterEl.textContent="0"; isDown=false; plankHoldTime=0; plankActive=false; goalAchieved=false;
    updateStreak(false);
    updateProgress();
    speak("Сброшено");
}
function startTimer() { startTime=Date.now(); if(timerInterval) clearInterval(timerInterval); timerInterval=setInterval(()=>{ if(isAnalyzing && !isPaused && timerEl) timerEl.textContent = Math.floor((Date.now()-startTime)/1000)+'с'; },1000); }
...
function toggleDailyQuest() {
    const el = document.getElementById('dailyQuest');
...
window.onload = () => {
    ...
    // Переключатели режима камера/видео
    ...
    if (modeCam) modeCam.onclick = () => { currentMode='camera'; if(video) video.classList.remove('hidden'); if(uploadedVideo) uploadedVideo.classList.add('hidden'); const uploadDiv = document.querySelector('.file-upload'); if(uploadDiv) uploadDiv.style.display='none'; showToast("Камера"); };
    if (modeVideo) modeVideo.onclick = () => { currentMode='video'; if(video) video.classList.add('hidden'); if(uploadedVideo) uploadedVideo.classList.remove('hidden'); const uploadDiv = document.querySelector('.file-upload'); if(uploadDiv) uploadDiv.style.display='block'; showToast("Видео"); };
    
    // Кнопки в модалке меню
